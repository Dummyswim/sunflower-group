...
def score_one_with_fundamental_check(
    df: pd.DataFrame,
    symbol: str,
    fundamental_data: Optional[Dict[str, float]] = None,
) -> Optional[Dict[str, float]]:
    """
    Wrap score_one() and apply simple fundamental adjustments.
    """
    met = score_one(df)
    if met is None:
        return None
    ...
    if not fundamental_data:
        met["fundamental_adjusted"] = False
        return met

    pat_yoy = fundamental_data.get("pat_yoy_pct", 0)
    profit_margin = fundamental_data.get("profit_margin", 15)
    debt_to_equity = fundamental_data.get("debt_to_equity", 2)
    npa_ratio = fundamental_data.get("npa_ratio", None)

    original_score = met["score"]
    alerts: List[str] = []
    strengths: List[str] = []
    ...
    if pat_yoy < -15:
        penalty = 0.15
        LOG.warning("%s: PROFIT CRASH (PAT %+.1f%% YoY); reducing score by %.3f", symbol, pat_yoy, penalty)
        met["score"] -= penalty
        alerts.append(f"Profit declined {pat_yoy:.1f}% YoY (critical concern)")
    elif pat_yoy > 0 and profit_margin < 10:
        penalty = 0.05
        LOG.warning("%s: Low margins (%.1f%%) despite positive growth; penalizing", symbol, profit_margin)
        met["score"] -= penalty
        alerts.append(f"Profit margins compressed to {profit_margin:.1f}% (low)")

    # Profit growth boosts
    if pat_yoy > 20:
        boost = 0.10
        LOG.info("%s: STRONG PROFIT GROWTH (PAT %+.1f%% YoY); boosting score by %.3f", symbol, pat_yoy, boost)
        met["score"] += boost
        strengths.append(f"Excellent profit growth {pat_yoy:.1f}% YoY")
    elif pat_yoy > 5:
        boost = 0.05
        LOG.info("%s: Solid profit growth (PAT %+.1f%% YoY); small boost by %.3f", symbol, pat_yoy, boost)
        met["score"] += boost
        strengths.append(f"Decent profit growth {pat_yoy:.1f}% YoY")

    # Leverage checks
    if debt_to_equity > 8:
        penalty = 0.15
        LOG.warning("%s: EXCESSIVE LEVERAGE (D/E %.1fx); major red flag", symbol, debt_to_equity)
        met["score"] -= penalty
        alerts.append(f"High leverage D/E {debt_to_equity:.1f}x (excessive)")
    elif debt_to_equity > 5:
        penalty = 0.10
        LOG.warning("%s: High leverage (D/E %.1fx); penalizing", symbol, debt_to_equity)
        met["score"] -= penalty
        alerts.append(f"Moderate-high leverage D/E {debt_to_equity:.1f}x")

    # Bank NPA
    if "BANK" in symbol.upper() and npa_ratio is not None:
        if npa_ratio > 2.0:
            penalty = 0.12
            LOG.warning("%s: High NPA %.2f%%; asset quality concern", symbol, npa_ratio)
            met["score"] -= penalty
            alerts.append(f"NPA ratio {npa_ratio:.2f}% (elevated, asset quality risk)")
        elif npa_ratio > 1.5:
            penalty = 0.05
            LOG.warning("%s: Moderate NPA %.2f%%", symbol, npa_ratio)
            met["score"] -= penalty
            alerts.append(f"NPA ratio {npa_ratio:.2f}% (monitoring needed)")

    met["score"] = max(0.0, min(1.0, met["score"]))

    met["pat_yoy_pct"] = pat_yoy
    met["profit_margin"] = profit_margin
    met["debt_to_equity"] = debt_to_equity
    met["npa_ratio"] = npa_ratio
    met["fundamental_data_date"] = fundamental_data.get("result_date", "unknown")

    score_adjustment = met["score"] - original_score
    met["score_adjustment"] = score_adjustment
    met["fundamental_adjusted"] = (score_adjustment != 0)

    if alerts:
        met["fundamental_alert"] = "; ".join(alerts)
    if strengths:
        met["fundamental_strength"] = "; ".join(strengths)

    if score_adjustment != 0:
        direction = "↑" if score_adjustment > 0 else "↓"
        LOG.info("%s: Score adjusted %s by %.3f (%.4f → %.4f)", symbol, direction, abs(score_adjustment), original_score, met["score"])
...
def write_markdown_with_fundamentals(df: pd.DataFrame, out_md: str, top_n: int) -> None:
    """
    Markdown output that highlights fundamental adjustments alongside technical scores.
    """
    lines: List[str] = []
    now = dt.datetime.now().strftime("%Y-%m-%d %H:%M")

    lines.append("# Swing Scan Findings and Recommendations (with Fundamental Intelligence)\n")
    lines.append(f"_Generated: {now}_\n")
    lines.append(
        "This report combines **technical setup** (EMA trends, resistance, volume) with **fundamental strength** (profit growth, margins, leverage).\n"
    )
    ...
    lines.append("## Summary\n")
    lines.append(f"- Total symbols scanned: **{len(df)}**\n")
    lines.append(f"- Top shortlist shown: **{len(top)}**\n")
    lines.append(f"- Generation time: {now}\n\n")

    actionable = top[top["actionable_state"] == "Actionable"]
    caution = top[top["actionable_state"] == "Actionable_Caution"]
    watch = top[top["actionable_state"] == "Watchlist"]

    def section(title: str, desc: str, subset: pd.DataFrame, caution_note: Optional[str] = None) -> None:
        lines.append(f"## {title}\n")
        lines.append(desc + "\n\n")
        if subset.empty:
            lines.append("_(None in this scan)_\n\n")
            return
        for _, r in subset.iterrows():
            sym = r["symbol"]
            score = r.get("score", np.nan)
            pat_yoy = r.get("pat_yoy_pct", np.nan)
            de = r.get("debt_to_equity", np.nan)
            alert = r.get("fundamental_alert", "")
            strength = r.get("fundamental_strength", "")
            ...
            if adj != 0:
                ...
                lines.append(f"({direction} {abs(adj):.3f})\n")
            else:
                lines.append("\n")

            if pd.notna(pat_yoy):
                lines.append(f"- **Profit Growth**: {pat_yoy:+.1f}% YoY\n")
            if pd.notna(de):
                lines.append(f"- **Leverage**: D/E {de:.1f}x\n")
            if strength:
                lines.append(f"- **Strength**: {strength}\n")
            if alert:
                lines.append(f"- **Alert**: {alert}\n")
            if caution_note:
                lines.append(f"- **Action**: {caution_note}\n")
            lines.append("\n")

    lines.append("---\n\n")
    section(
        "🟢 ACTIONABLE (Ready to Enter)",
        "These broke out on a daily close with strong volume + fundamentals are supportive.",
        actionable,
    )
    lines.append("---\n\n")
    section(
        "🟡 ACTIONABLE WITH CAUTION",
        "Breakout signal present, but caution flags exist (extended price, overhead resistance, or deteriorating fundamentals).",
        caution,
        caution_note="Wait 1-2 days for pullback or hold above resistance; consider smaller size.",
    )
    lines.append("---\n\n")
    section(
        "🔵 WATCHLIST (Near Resistance, Waiting for Volume Confirmation)",
        "Close to 20D resistance. Wait for daily close above resistance with 2× average volume, then enter.",
        watch,
    )

    lines.append("---\n\n## Detailed Metrics (Top Shortlist)\n\n")
    cols = [
        "symbol",
        "asset_bucket",
        "actionable_state",
    ...
    for _, r in top.iterrows():
        ...
        for c in cols:
            v = r.get(c, np.nan)
            if c in {"score", "pat_yoy_pct", "profit_margin", "debt_to_equity", "adx14", "rsi14"}:
                row.append(f"{v:.2f}" if pd.notna(v) else "—")
            elif c == "close":
                row.append(f"{v:.2f}" if pd.notna(v) else "—")
            elif c == "turnover":
                row.append(f"{v:.2e}" if pd.notna(v) else "—")
            else:
                row.append(str(v))
        lines.append("| " + " | ".join(row) + " |\n")

    lines.append("\n---\n\n## Methodology\n\n")
    lines.append("**Technical Scoring**:\n")
    lines.append("- Trend: EMA50 > EMA200 + close > EMA50\n")
    lines.append("- Proximity: Distance to 20D high (< 6% = hot)\n")
    lines.append("- Squeeze: Bollinger Band width + ATR contraction\n")
    lines.append("- Volume: Confirmation on 2× average\n")
    lines.append("- Risk: 1.5 ATR stops, 2:1 reward:risk targets\n\n")

    lines.append("**Fundamental Adjustments**:\n")
    lines.append("- Profit crash (PAT < -15% YoY): Score -0.15\n")
    lines.append("- Margin compression (<10% with growth): Score -0.05\n")
    lines.append("- Strong growth (PAT > 20% YoY): Score +0.10\n")
    lines.append("- High leverage (D/E > 8x): Score -0.15\n")
    lines.append("- Bank NPA (>2%): Score -0.12\n\n")

    lines.append("**Data Sources**:\n")
    lines.append("- OHLCV: Dhan v2 API (1-year historical)\n")
    lines.append("- Fundamentals: Q2 FY26 POC data\n")
    lines.append(f"- Last updated: {now}\n\n")

    lines.append("**Disclaimer**: Swing trading scanner with fundamental filters. Not financial advice.\n")
...
def main() -> int:
    ...
    fund_fetcher = SimpleFundamentalFetcher(cache_dir=Path("cache_fundamental"))
    LOG.info("Fundamental fetcher initialized (Q2 FY26 POC data)")
    ...
    for i, sym in enumerate(watch, start=1):
        ...
        try:
            ...
            # Fundamental overlay
            fund_data = fund_fetcher.fetch(sym, max_age_hours=24)
            if fund_data:
                met = score_one_with_fundamental_check(df, symbol=sym, fundamental_data=fund_data)
                if met is None:
                    failures.append((sym, "Fundamental check failed"))
                    continue
    ...
    # Ensure fundamental columns exist for consistent output
    fundamental_cols = [
        "pat_yoy_pct",
        "profit_margin",
        "debt_to_equity",
    ...
    out_df.reindex(columns=cols_to_write).to_csv(args.out_csv, index=False)
    LOG.info("Wrote CSV report: %s (rows=%d)", args.out_csv, len(out_df))

    write_markdown_with_fundamentals(out_df, args.out_md, args.top_n)
