...
Sidecar VWAP + CVD calculator for NIFTY Bank futures (Dhan WebSocket v2).

- Connects to DhanHQ live market feed as a separate process.
- Subscribes to the configured front-month NIFTY Bank future (SecurityId from FUT_SECURITY_ID env).
...
def _subscription_payload(cfg: SidecarConfig) -> Dict[str, Any]:
    """
    Subscribe to one instrument (NIFTY BANK FUT) using FULL PACKET.
...
async def _async_main():
    ...
    logger.info("=" * 60)
    logger.info("STARTING NIFTY BANK FUTURES VWAP + CVD SIDECAR")
    logger.info("=" * 60)
