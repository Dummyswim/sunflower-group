...
Sidecar VWAP + CVD calculator for SENSEX futures (Dhan WebSocket v2).

- Connects to DhanHQ live market feed as a separate process.
- Subscribes to the configured front-month SENSEX future (SecurityId from FUT_SECURITY_ID env).
...
class SidecarConfig:
    ...
    exchange_segment: str = "BSE_FNO"     # futures segment
...
def _subscription_payload(cfg: SidecarConfig) -> Dict[str, Any]:
    """
    Subscribe to one instrument (SENSEX FUT) using FULL PACKET.
...
async def _async_main():
    ...
    logger.info("=" * 60)
    logger.info("STARTING SENSEX FUTURES VWAP + CVD SIDECAR")
    logger.info("=" * 60)
