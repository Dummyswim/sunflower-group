...
try:
    ...
    def setup_logging2(*args: Any, **kwargs: Any) -> None:
        logging.basicConfig(level=logging.INFO, format='%(asctime)s %(levelname)s %(message)s')
    def start_dynamic_level_watcher(*args: Any, **kwargs: Any) -> None:
        return
...
class FuturesVWAPCVDClient:
    ...
    def __init__(self, cfg: SidecarConfig, logger: logging.Logger):
        ...
        # Futures-specific extra fields (from Full packet)
        self._last_total_sell_qty: Optional[float] = None
        self._last_total_buy_qty: Optional[float] = None
        self._last_oi: Optional[float] = None
        self._last_oi_high_day: Optional[float] = None
        self._last_oi_low_day: Optional[float] = None
        self._last_depth_imb: Optional[float] = None
        self._last_spread: Optional[float] = None
        self._last_microprice: Optional[float] = None
        self._prev_candle_oi: Optional[float] = None
        self._candle_last_sell_qty: float = float("nan")
        self._candle_last_buy_qty: float = float("nan")
        self._candle_last_oi: float = float("nan")
        self._candle_last_oi_high: float = float("nan")
        self._candle_last_oi_low: float = float("nan")
        self._candle_last_doi: float = float("nan")
        self._candle_last_depth_imb: float = float("nan")
        self._candle_last_spread: float = float("nan")
        self._candle_last_microprice: float = float("nan")
    ...
    def _parse_ticker_packet(self, data: bytes) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            ts = self._epoch_to_ts(int(ltt_raw), recv_ts)
    ...
    def _parse_quote_or_full_packet(self, data: bytes) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            extra: Dict[str, Any] = {}
            if code == self.FULL_PACKET and len(data) >= 46:
                # Full packet additional fields (NSE_FNO derivatives include OI + depth)
                extra["total_sell_qty"] = float(struct.unpack("<I", data[26:30])[0])
                extra["total_buy_qty"]  = float(struct.unpack("<I", data[30:34])[0])
                extra["oi"]            = float(struct.unpack("<I", data[34:38])[0])
                extra["oi_high_day"]   = float(struct.unpack("<I", data[38:42])[0])
                extra["oi_low_day"]    = float(struct.unpack("<I", data[42:46])[0])

                # Depth (5 levels x 20 bytes) starts at byte 63 per docs => offset 62 (0-based)
                if len(data) >= 162:
                    try:
                        depth_off = 62
                        bid_sum = 0.0
                        ...
                        for lvl in range(5):
                            o = depth_off + (lvl * 20)
                            bid_qty = float(struct.unpack("<I", data[o:o+4])[0])
                            ask_qty = float(struct.unpack("<I", data[o+4:o+8])[0])
                            # o+8:o+12 = order counts (ignored)
                            bid_px = float(struct.unpack("<f", data[o+12:o+16])[0])
                            ask_px = float(struct.unpack("<f", data[o+16:o+20])[0])
                            ...
                            bid_sum += bid_qty
                            ask_sum += ask_qty
                        denom = max(1e-9, bid_sum + ask_sum)
                        extra["depth_imb"] = float((bid_sum - ask_sum) / denom)
                        if np.isfinite(best_bid) and np.isfinite(best_ask):
                            extra["spread"] = float(max(0.0, best_ask - best_bid))
                            mp_denom = max(1e-9, best_bid_qty + best_ask_qty)
                            extra["microprice"] = float((best_bid * best_ask_qty + best_ask * best_bid_qty) / mp_denom)
                    except Exception:
                        # Depth is optional; never fail the tick if it doesn't parse.
                        pass
    ...
    def _update_from_tick(self, tick: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Cache futures-only extra fields from Full packet (if present)
        if tick.get("packet_type") == "full":
            for k, attr in [
                ("total_sell_qty", "_last_total_sell_qty"),
                ("total_buy_qty", "_last_total_buy_qty"),
                ("oi", "_last_oi"),
                ("oi_high_day", "_last_oi_high_day"),
                ("oi_low_day", "_last_oi_low_day"),
                ("depth_imb", "_last_depth_imb"),
                ("spread", "_last_spread"),
                ("microprice", "_last_microprice"),
            ]:
                if k in tick:
                    v = self._safe_float(tick.get(k), default=float("nan"))
                    setattr(self, attr, v)
    ...
    def _update_candle(self, ts: datetime, ltp: float, dvol: float, session_vwap: float, cvd_val: float) -> None:
        ...
        # Snapshot the most recent futures extra fields into this candle (last-known within the minute)
        self._candle_last_sell_qty = self._safe_float(self._last_total_sell_qty, default=float("nan"))
        self._candle_last_buy_qty  = self._safe_float(self._last_total_buy_qty, default=float("nan"))
        self._candle_last_oi       = self._safe_float(self._last_oi, default=float("nan"))
        self._candle_last_oi_high  = self._safe_float(self._last_oi_high_day, default=float("nan"))
        self._candle_last_oi_low   = self._safe_float(self._last_oi_low_day, default=float("nan"))
        self._candle_last_depth_imb = self._safe_float(self._last_depth_imb, default=float("nan"))
        self._candle_last_spread    = self._safe_float(self._last_spread, default=float("nan"))
        self._candle_last_microprice = self._safe_float(self._last_microprice, default=float("nan"))
    ...
    def _flush_candle(self) -> None:
        ...
        try:
            # dOI (per candle): last OI - previous candle OI
            oi_now = self._safe_float(self._candle_last_oi, default=float("nan"))
            if self._prev_candle_oi is None or not np.isfinite(self._prev_candle_oi):
                self._candle_last_doi = float("nan")
            else:
                self._candle_last_doi = float(oi_now - float(self._prev_candle_oi))
            if np.isfinite(oi_now):
                self._prev_candle_oi = float(oi_now)
            ...
            row = [
                ...
                f"{self._safe_float(self._candle_last_sell_qty):.2f}",
                f"{self._safe_float(self._candle_last_buy_qty):.2f}",
                f"{self._safe_float(self._candle_last_oi):.2f}",
                f"{self._safe_float(self._candle_last_oi_high):.2f}",
                f"{self._safe_float(self._candle_last_oi_low):.2f}",
                f"{self._safe_float(self._candle_last_doi):.2f}",
                f"{self._safe_float(self._candle_last_depth_imb):.6f}",
                f"{self._safe_float(self._candle_last_spread):.4f}",
                f"{self._safe_float(self._candle_last_microprice):.4f}",
            ]
            line = ",".join(row)
        ...
        self._candle_last_sell_qty = float("nan")
        self._candle_last_buy_qty = float("nan")
        self._candle_last_oi = float("nan")
        self._candle_last_oi_high = float("nan")
        self._candle_last_oi_low = float("nan")
        self._candle_last_doi = float("nan")
        self._candle_last_depth_imb = float("nan")
        self._candle_last_spread = float("nan")
        self._candle_last_microprice = float("nan")
    ...
    def _append_tick_row(
        ...
        try:
            row = [
                ...
                f"{self._safe_float(self._last_total_sell_qty, default=float('nan')):.2f}",
                f"{self._safe_float(self._last_total_buy_qty, default=float('nan')):.2f}",
                f"{self._safe_float(self._last_oi, default=float('nan')):.2f}",
                f"{self._safe_float(self._last_depth_imb, default=float('nan')):.6f}",
                f"{self._safe_float(self._last_spread, default=float('nan')):.4f}",
                f"{self._safe_float(self._last_microprice, default=float('nan')):.4f}",
            ]
            line = ",".join(row)
