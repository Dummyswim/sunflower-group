...
class TradeBrainConfig:
    ...
    dup_arm_to_main: bool = os.getenv("TB_DUP_ARM_TO_MAIN", "1") == "1"

    # Optional: Futures sidecar (Option C). Read once per 1m candle close.
    use_fut_flow: bool = os.getenv("TB_USE_FUT_FLOW", "1") == "1"
    fut_sidecar_path: str = os.getenv("TB_FUT_SIDECAR_PATH", "trained_models/production/fut_candles_vwap_cvd.csv")
...
class TradeBrain:
        ...
        try:
            ...
            with self._lock:
                ...
                if self.pos is not None:
                    exited = self._check_intra_tick_exits(ltp, ts, recv_ms=recv_ms)
                if exited:
                    return
    ...
    def _close_candle_locked(self) -> None:
        ...
        if bool(getattr(self.cfg, "use_fut_flow", False)):
            line = _tail_last_line(str(getattr(self.cfg, "fut_sidecar_path", "")))
            fut = _parse_fut_candle_row(line) if line else None
    ...
    def _execute_entry(
        ...
        *,
        ...
        is_runner = bool(m.get("is_runner", False))
        if is_runner:
            tp = None
    ...
    def _check_intra_tick_exits(self, ltp: float, ts: datetime, *, recv_ms: int) -> bool:
        """Tick-level hard stop, SL/TP fills, BE + trailing."""
        p = self.pos
        if p is None:
            return False

        def _do_exit(suggestion: str, px: float, reason: str = "") -> bool:
            payload = self._execute_exit(suggestion, px=px, ts=ts, reason=reason)
            self._mark_rearm_required(payload.get("engine", "unknown"), exit_ms=recv_ms)
            return True
    ...
    def _write_signal(self, decision: Dict[str, Any], metrics: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if (not self.cfg.write_all) and sug0.startswith("HOLD"):
                return
            ...
            if is_arm:
                ...
                with self._arm_write_lock:
                    self.arm_jsonl_file.write(json.dumps(payload, default=str) + "\n")
                    self.arm_jsonl_file.flush()
                ...
                if bool(getattr(self.cfg, "dup_arm_to_main", False)):
                    ...
                    payload2["stream"] = "arm"
                    with self._write_lock:
                        self.jsonl_file.write(json.dumps(payload2, default=str) + "\n")
                        self.jsonl_file.flush()
            else:
