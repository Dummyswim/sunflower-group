...
## Action Now / Defer
Action now:
- Revisit HTF veto thresholds/conditional override (BUY misses still dominated by `no_direction` / `htf_veto`).

Defer:
- Relax setup gating in trend lanes (not the top blocker in recent sessions).
- Review policy gating in trend regimes (tradeable is still near zero; fix flow gates first).
