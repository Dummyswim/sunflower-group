    ...
    async def connect(self):
        ...
        while attempt < max_attempts and self.running:
            try:
                ...
                logger.error(f"Connection attempt {attempt} failed (server closed): {e}") 
        ...
        try:
            ...
            subscription = {
                ...
                "InstrumentList": [{
                    ...
                    "SecurityId": str(self.config.nifty_security_id)  # str, not int as per API DOC
                }]
            }  
            ...
            try: 
                ...
                # Guard: do not send if socket already closed by server
                if not self.websocket or getattr(self.websocket, "closed", True):
                    raise RuntimeError("WebSocket not open during subscribe send")
                ...
                # Short grace for initial packets
                await asyncio.sleep(2)


                # Probe market status only if still open and no ticks yet
                if self.tick_count == 0:
                    logger.warning("[Subscribe] No ticks received after subscription - checking market status")
                    status_request = {"RequestCode": 7}
                    if not getattr(self.websocket, "cl[Subscribe] WebSocket state: unknown (attributes missing)osed", True):
                ...
                logger.error(f"[Subscribe] Server closed connection during subscribe: {e}")
                raise
        except Exception as e:
