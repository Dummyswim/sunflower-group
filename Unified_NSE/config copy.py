...
class UnifiedTradingConfig:
    ...
    # ADD these new parameters after the existing 1m engine config
    micro_window_sec_1m: int = 25
    micro_min_ticks_1m: int = 30
    next_minute_imb_weight: float = 55.0  # up from 45.0
    next_minute_slope_weight: float = 10.0  # down from 17.5
    next_minute_mtf_extreme_pos_hi: float = 0.95  # was 0.98
    next_minute_mtf_extreme_pos_lo: float = 0.05
    next_minute_drift_guard_pct: float = 0.01  # was 0.02
    next_minute_micro_override_imb: float = 0.50  # keep
    next_minute_micro_override_slope: float = 0.25  # keep



    # ADD these new dual-window parameters
    # Short micro window for boundary-reversal detection
    micro_short_window_sec_1m: int = 8
    micro_short_min_ticks_1m: int = 12

    # Blending and penalties
    next_minute_imb_short_weight: float = 0.60
    next_minute_imb_long_weight: float = 0.40
    next_minute_reversal_penalty_pp: float = 8.0   # per disagreement (short vs long)
    next_minute_reversal_penalty_cap_pp: float = 14.0

    # Keep earlier tuned weights/guards you already added:
    # next_minute_imb_weight=55.0, next_minute_slope_weight=10.0, etc.
