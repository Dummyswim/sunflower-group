...
async def run_quote_feed(cfg: DhanConfig, tb: Optional[TickHandler], log: logging.Logger) -> None:
...
async def run_system() -> None:
    ...
    tb: Optional[TickHandler] = None
    if run_tb:
        try:
            from tradebrain import TradeBrain, TradeBrainConfig, setup_logger
        except Exception as e:
            log.warning("[DHAN] TradeBrain import failed (%s). Continuing without TradeBrain.", e)
            run_tb = False
